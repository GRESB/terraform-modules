...
variable "user_pool_alias_attributes" {
  description = "The user pool attributes used for identifying users."
  type        = list(string)
  default     = ["email"]
}
...
variable "user_pool_custom_attributes" {
  description = "Custom attributes to be configured on the user pool."
  type = map(object({
    name                     = string
  ...
  validation {
    condition = alltrue([for a in values(var.user_pool_custom_attributes) :
      (a.attribute_data_type == "String" && a.string_attribute_constraints != null)
    || (a.attribute_data_type == "Number" && a.number_attribute_constraints != null)])
...
variable "remember_device" {
  description = "Should the user pool store information about user devices."
  type        = bool
  default     = false
}
